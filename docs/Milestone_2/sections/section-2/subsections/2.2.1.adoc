...
|REQ-AUTH-F-01 |Authenticate users, create accounts, and enforce role-based access (patient, doctor, admin). Provide clear failure reasons and lockout on repeated failures. |E1 -> F1.1, F1.4
|REQ-PROF-F-01 |Persist patient and doctor profile data (name, contact, specialty/clinic, accessibility prefs). Validate required fields, allow users to update their own profiles, and audit changes. |E1 -> F1.2, F1.3
|REQ-AVAIL-F-01 |Allow providers to declare weekly availability and ad-hoc slots; prevent overlapping slots and surface conflicts to the user. |E2 -> F2.1, F2.2, F2.3
|REQ-BOOK-F-01 |Let patients book available slots, reserve the slot atomically, and prevent double booking. |E3 -> F3.2, F3.3, F3.4
|REQ-BOOK-F-02 |Allow patients to cancel upcoming appointments and immediately release the slot for reuse. |E3 -> F3.3
|REQ-NOTIF-F-01 |Send confirmation notifications after successful booking, including appointment details. |E4 -> F4.1
|REQ-NOTIF-F-02 |Send reminder notifications before the appointment based on user preferences. |E4 -> F4.2, F4.3
|REQ-ADMIN-F-01 |Provide admins with read-only oversight dashboards (appointments, users, activity) with filtering/export; enforce least-privilege access. |E5 -> F5.1, F5.2
|===
